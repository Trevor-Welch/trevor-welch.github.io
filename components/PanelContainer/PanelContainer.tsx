...
export default function PanelContainer({ items, defaultId, className = '' }: PanelContainerProps) {
  ...
  return (
    <div
      className={`flex flex-col items-start md:flex-row gap-8 md:gap-0 w-full mx-auto min-h-[500px] ${className} ${styles.panelContainer}`}
    >
      {/* Left menu */}
      <div className="flex items-stretch w-full md:w-1/3 pr-0 md:pr-6">
        ...
        >
          <div className="w-[12px] bg-[var(--color-text)] h-full" />
          <div className="w-[4px] bg-[var(--color-text)] h-full" />
        </div>
        {/* Menu list */}
      ...
      {/* Right panel */}
      <div className="w-full md:w-2/3 max-w-full md:max-w-2/3">
        <div
          ref={panelRef}
          className={`flex flex-col items-start text-[var(--color-text)] bg-[var(--color-bg)] shadow-lg ${
            showContent ? styles.fadeShow : styles.fadeHide
          }`}
          style={{
            width: 'fit-content',
            maxWidth: '100%',
            height: finalHeight,
            transition: 'width 0.3s ease, height 0.1s ease, opacity 0.5s ease-in-out',
            overflow: 'hidden',
